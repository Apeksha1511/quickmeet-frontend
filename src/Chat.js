...
// Single shared socket — same instance as Video.js
const socket = io(process.env.REACT_APP_SERVER_URL || "http://localhost:5000");

function Chat({ roomId, name }) {
  ...
  useEffect(() => {
    ...
    // Receive new messages from anyone (including yourself — server echoes to all)
    socket.on("receive-message", (data) => {
      setMessages((prev) => {
        // avoid duplicates if history already has it
        return [...prev, { ...data, self: data.name === name }];
      });
    });
  ...
  return (
    <>
      <style>{`
        ...
        .chat-header-icon {
          width: 26px; height: 26px; background: rgba(26,111,255,0.15);
          border-radius: 7px; display: flex; align-items: center; justify-content: center; font-size: 13px;
        }
        .chat-header-title { font-size: 14px; font-weight: 500; color: rgba(255,255,255,0.85); }
        .chat-count {
          margin-left: auto; background: rgba(255,255,255,0.07);
          border-radius: 20px; padding: 2px 9px; font-size: 11px; color: rgba(255,255,255,0.35);
        }
        .chat-messages {
        ...
        .chat-empty {
          ...
          flex-direction: column; gap: 8px; color: rgba(255,255,255,0.2); font-size: 13px;
        }
        ...
        .chat-input {
          ...
          transition: border-color 0.18s; max-height: 100px;
        ...
        .send-btn {
          ...
          background: linear-gradient(135deg, #1a6fff, #0095ff); color: #fff;
          font-size: 16px; cursor: pointer; transition: all 0.18s;
          display: flex; align-items: center; justify-content: center; flex-shrink: 0;
          box-shadow: 0 4px 12px rgba(26,111,255,0.35);
        }
        .send-btn:hover { transform: scale(1.07); box-shadow: 0 6px 18px rgba(26,111,255,0.5); }
      ...
      <div className="chat-root">
        ...
        <div className="chat-messages">
          {messages.length === 0 ? (
            ...
            messages.map((m, i) => (
              <div key={i} className={`msg-wrapper ${m.self ? "self" : "other"}`}>
                {!m.self && <span className="msg-sender">{m.name}</span>}
                <div className={`msg-bubble ${m.self ? "msg-self" : "msg-other"}`}>
                  {m.message}
                </div>
